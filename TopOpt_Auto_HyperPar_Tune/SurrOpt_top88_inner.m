...
function [minc,volfin,gray] = SurrOpt_top88_inner(nelx,nely,volfrac,penal,rmin,ft)
...
%     if change<= 0.001
%       figure(1)
%       export_fig('Figures/1lengthscale_150_50_rmin_2','-eps')
%     end 
...
%Re-Analyze final result with penal = 1
  penal = 1;
  sK = reshape(KE(:)*(Emin+xPhys(:)'.^penal*(E0-Emin)),64*nelx*nely,1);
  K = sparse(iK,jK,sK); K = (K+K')/2;
  U(freedofs) = K(freedofs,freedofs)\F(freedofs);
